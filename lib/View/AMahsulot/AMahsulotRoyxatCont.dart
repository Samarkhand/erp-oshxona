...
import 'package:erp_oshxona/View/AMahsulot/AMahsulotRoyxatView.dart';
import 'package:erp_oshxona/Model/aMahsulot.dart';
import 'package:erp_oshxona/Model/system/controller.dart';

class AMahsulotRoyxatCont with Controller {
  late AMahsulotRoyxatView widget;
  List objectList = [];
  late DateTime sanaD;
  late DateTime sanaG;
  ...
  Future<void> delete(AMahsulot element) async {
    await AMahsulot.service!.deleteId(element.tr);
  ...
  Future<void> loadItems() async {
    (await AMahsulot.service!.select(where: "  sana>=${toSecond(sanaD.millisecondsSinceEpoch)} AND sana<=${toSecond(sanaG.millisecondsSinceEpoch)} ORDER BY sana DESC")).forEach((key, value) {
  ...
  loadFromGlobal(){
    objectList = AMahsulot.obyektlar.values.toList();
